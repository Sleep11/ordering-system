  ...
  <header class="topbar">
    <div class="topbar-inner">
      <div class="topbar-left">
        <button class="btn btn-icon topbar-menu-btn" @click="toggleSidebar" title="切换侧边栏">
      ...
      <div class="topbar-right">
        ...
        <span v-if="auth.isAdmin" class="version-badge">v3.0.0.1</span>
...
async function handleClearOrders() {
  if (clearState.value === 0) {
    clearState.value = 1
    setTimeout(() => { if (clearState.value === 1) clearState.value = 0 }, 5000)
    return
  }
  if (clearState.value === 1) {
    ...
    clearState.value = 0
    if (result.success) {
      toast.show(result.message || '订单已清除', 'success')
      await orders.refreshNow()
    } else {
