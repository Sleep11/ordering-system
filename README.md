...
| `app.js` | ~1520 | 前端核心逻辑：状态管理、API 调用、UI 渲染、事件处理 |
| `styles.css` | ~1280 | 完整样式系统：设计令牌、布局、组件、动效、响应式 |
| `api.node.js` | ~820 | 后端 API 路由：认证、订单 CRUD、用户管理、设置 |
...
- **确认对话框激活**: 自动跳过刷新，防止 DOM 重建打断确认操作
- **滚动位置**: 刷新时自动保持当前滚动位置
