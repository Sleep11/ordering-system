<?php
// 多人在线点餐系统 — PHP API v3
// Using minimized output, no http_response_code
...
// ---- Auth ----
function mkSalt() { return bin2hex(random_bytes(16)); }
function hashPw($pw, $s) { return hash('sha256', $s.$pw); }
function checkPw($pw, $s, $h) { return hashPw($pw, $s) === $h; }
function mkToken($uid, $role) {
    $exp = time() + 7*86400;
    $p = $uid.':'.$role.':'.$exp;
    return base64_encode($p.':'.hash_hmac('sha256', $p, 'k2026'));
}
function chkToken($tok) {
    if(!$tok) return null;
    $d = base64_decode($tok); if(!$d) return null;
    $p = explode(':', $d); if(count($p) < 4) return null;
    if(hash_hmac('sha256', $p[0].':'.$p[1].':'.$p[2], 'k2026') !== $p[3]) return null;
    if(time() > intval($p[2])) return null;
    return ['id'=>$p[0], 'role'=>$p[1]];
}
function curUser() {
    $t = $_POST['token'] ?? $_GET['token'] ?? '';
    return $t ? chkToken($t) : null;
}
function needAuth() {
    $u = curUser(); if(!$u) err('未登录', 401); return $u;
}
function needAdmin() {
    $u = needAuth(); if($u['role'] !== 'admin') err('无权限', 403); return $u;
}
function getUsers() {
    $u = kvjson('users');
...
elseif($a === 'me') {
    $u = curUser(); if(!$u) err('未登录', 401);
    foreach(getUsers() as $x) { if($x['id'] === $u['id']) { ok(['user'=>['id'=>$x['id'], 'name'=>$x['name'], 'role'=>$x['role']]]); return; } }
    err('用户不存在', 404);
}
...
elseif($a === 'get-orders') {
    ...
    foreach(kvkeys() as $k) {
        ...
        $p = explode('_', $k); if(count($p) >= 2 && isset($ds[$p[1]])) { $o = kvjson($k); if($o) $orders[] = $o; }
    }
    ok(['orders'=>$orders]);
}
elseif($a === 'create-order') {
    ...
    if(is_array($ri) && !empty($ri)) {
        $tp = 0; $ins = [];
        foreach($ri as $r) {
            $q = max(1, intval($r['quantity'] ?? 1));
            if(!empty($r['menuId'])) { $mm = findMenuItem($r['menuId']); if(!$mm) err('餐品不存在', 404); $up = floatval($mm['price']); $nm = $mm['name']; }
            else { $up = floatval($r['price'] ?? 0); $nm = $r['name'] ?? ''; }
            $its[] = ['menuId'=>$r['menuId']??'', 'name'=>$nm, 'price'=>$up, 'quantity'=>$q];
            $tp += $up * $q; $ins[] = $nm.($q > 1 ? '×'.$q : '');
        }
...
elseif($a === 'delete-order') {
    $u = needAuth(); $oid = $_POST['orderId'] ?? ''; if(!$oid) err('订单ID不能为空', 400);
    $o = kvjson($oid); if(!$o) err('订单不存在', 404);
    if($u['role'] !== 'admin' && $o['userId'] !== $u['id']) err('无权限', 403);
    kvdel($oid); ok(null, '订单已删除');
}
elseif($a === 'delete-orders-by-date') {
    needAdmin(); $dt = $_POST['date'] ?? ''; if(!$dt) err('日期不能为空', 400);
    $dl = 0; $pf = 'order_'.$dt.'_';
    foreach(kvkeys() as $k) { if(strpos($k, $pf) === 0) { kvdel($k); $dl++; } }
    ok(null, "已删除 $dl 条订单");
}
elseif($a === 'update-payment') {
    needAdmin(); $oid = $_POST['orderId'] ?? ''; $pd = in_array($_POST['paid'] ?? '', ['true', '1'], true);
    if(!$oid) err('订单ID不能为空', 400);
    $o = kvjson($oid); if(!$o) err('订单不存在', 404);
    $o['paid'] = $pd; $o['actual'] = $pd ? (floatval($o['price']) ?: 0) : 0; $o['updatedAt'] = date('c');
    kvsetj($oid, $o); ok(null, $pd ? '已标记为已付' : '已取消已付');
}
elseif($a === 'refund-order') {
    needAdmin(); $oid = $_POST['orderId'] ?? ''; if(!$oid) err('订单ID不能为空', 400);
    $o = kvjson($oid); if(!$o) err('订单不存在', 404);
    $o['refunded'] = true; $o['refundedAt'] = date('c'); $o['updatedAt'] = date('c');
    kvsetj($oid, $o); ok(null, '已退款');
}
elseif($a === 'get-settings') {
    $lp = kvget('settings_blind_lunch_price'); $dp = kvget('settings_blind_dinner_price');
    ok(['settings'=>[
        'orderLocked'=>kvget('settings_order_locked')==='true',
        'lunchLocked'=>kvget('settings_lunch_locked')==='true',
        'dinnerLocked'=>kvget('settings_dinner_locked')==='true',
        'lunchSelfPick'=>kvget('settings_lunch_selfpick')==='true',
        'dinnerSelfPick'=>kvget('settings_dinner_selfpick')==='true',
        'blindLunchPrice'=>$lp?floatval($lp):11,
        'blindDinnerPrice'=>$dp?floatval($dp):12
    ]]);
}
elseif($a === 'update-settings') {
    needAdmin(); $k = $_POST['key'] ?? ''; $v = $_POST['value'] ?? '';
    if(!$k) err('设置项不能为空', 400);
    kvset($k, $v);
    if($k === 'settings_lunch_selfpick' || $k === 'settings_dinner_selfpick') {
        $mt = ($k === 'settings_lunch_selfpick') ? 'lunch' : 'dinner'; $en = ($v === 'true'); $td = chinaDate();
        foreach(kvkeys() as $x) {
            if(strpos($x, 'order_') !== 0) continue; $p = explode('_', $x); if(count($p) < 4) continue;
            if($p[1] !== $td || end($p) !== $mt) continue;
            $o = kvjson($x); if(!$o) continue;
            $o['discount'] = $en ? 1 : 0; $o['receivable'] = max(0, floatval($o['price']) - $o['discount']);
            $o['actual'] = ($o['paid']??false) ? floatval($o['price']) : 0; $o['refund'] = ($o['paid']??false) ? $o['discount'] : 0;
            $o['updatedAt'] = date('c'); kvsetj($x, $o);
        }
    }
    ok(null, '设置已更新');
}
elseif($a === 'get-report') {
    needAuth(); $tp = $_POST['type'] ?? $_GET['type'] ?? 'week'; $off = intval($_POST['offset'] ?? $_GET['offset'] ?? 0);
    if($tp === 'week') {
        $nw = new DateTime('now', new DateTimeZone('Asia/Shanghai'));
        $dow = intval($nw->format('N')) - 1;
        $st = (clone $nw)->modify('-'.$dow.' days')->modify($off * 7 .' days')->setTime(0,0);
        $ed = (clone $st)->modify('+6 days')->setTime(23,59,59);
    } else {
        $nw = new DateTime('now', new DateTimeZone('Asia/Shanghai'));
        $y = intval($nw->format('Y')); $m = intval($nw->format('m')) + $off;
        while($m < 1) { $m += 12; $y--; } while($m > 12) { $m -= 12; $y++; }
        $st = new DateTime("$y-$m-01", new DateTimeZone('Asia/Shanghai'));
        $ed = (clone $st)->modify('+1 month -1 day');
    }
    $df = $st->format('Y-m-d'); $dt2 = $ed->format('Y-m-d');
    $to = 0; $po = 0; $ta = 0; $pa = 0; $ps = [];
    foreach(kvkeys() as $k) {
        if(strpos($k, 'order_') !== 0) continue; $p = explode('_', $k); if(count($p) < 2) continue;
        $d = $p[1]; if($d < $df || $d > $dt2) continue;
        $o = kvjson($k); if(!$o) continue;
        $rec = floatval($o['receivable'] ?? max(0, floatval($o['price']??0) - floatval($o['discount']??0)));
        $to++; $ta += $rec;
        if($o['paid'] ?? false) { $po++; $pa += floatval($o['actual'] ?? $o['price'] ?? 0); }
        $nm = $o['personName'] ?? '';
        if(!isset($ps[$nm])) $ps[$nm] = ['name'=>$nm, 'count'=>0, 'amount'=>0];
        $ps[$nm]['count']++; $ps[$nm]['amount'] += $rec;
    }
    ok(['dateFrom'=>$df, 'dateTo'=>$dt2, 'totalOrders'=>$to, 'paidOrders'=>$po, 'totalAmount'=>round($ta,2), 'paidAmount'=>round($pa,2), 'personStats'=>array_values($ps)]);
}
elseif($a === 'get-menu') {
    needAuth(); $m = getMenu();
    usort($m, function($a, $b) { return ($b['weight']??0) - ($a['weight']??0); });
    ok(['menu'=>$m]);
}
elseif($a === 'update-menu') {
    needAdmin(); $m = $_POST['menu'] ?? ''; $mj = $_POST['menuJson'] ?? '';
    if(is_string($m)) $m = json_decode($m, true);
    if(!is_array($m) && is_string($mj)) $m = json_decode($mj, true);
    if(!is_array($m)) err('菜单数据格式错误', 400);
    foreach($m as $x) if(empty($x['id']) || empty($x['name']) || !isset($x['price'])) err('菜单项缺少必要字段', 400);
    $cm = getMenu();
    foreach($m as &$x) { if(!isset($x['note'])) { foreach($cm as $c) if($c['id'] === $x['id'] && !empty($c['note'])) { $x['note'] = $c['note']; break; } } }
    kvsetj('settings_menu', $m); ok(null, '菜单更新成功');
}
...
elseif($a === 'restore-kv') {
    needAdmin(); $d = $_POST['data'] ?? ''; $ow = ($_POST['overwrite'] ?? '') === 'true';
    if(!$d) err('数据不能为空', 400);
    $kv = json_decode($d, true); if(!is_array($kv)) err('数据格式错误', 400);
    $rs = 0; foreach($kv as $k=>$v) { if(!$ow && kvget($k) !== null) continue; kvset($k, is_string($v) ? $v : json_encode($v, JSON_UNESCAPED_UNICODE)); $rs++; }
    ok(null, "已恢复 $rs 条数据");
}
else {
    err('未知操作: '.$a, 400);
